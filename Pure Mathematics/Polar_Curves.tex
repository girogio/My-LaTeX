...
\begin{example}
Sketch the graph of $2+2\cos\theta$
\end{example}
\begin{center}
\resizebox{300pt}{!}{%
...
\begin{center}
\begin{tikzpicture}
\draw[thick,->,>=latex] (-3,0)--(4,0) node[above] {$x$};
\draw[thick,->,>=latex] (0,-3)--(0,4) node[left] {$y$};
\draw[domain=0:540,scale=0.7,samples=1000] plot (\x:{2+(2*cos(\x))});
\draw[red,  thick, , domain=-1:1.5] plot(\x, {tan((3*pi)/4) r)*\x});
\end{tikzpicture}	
\end{center}

\begin{example}
	Sketch the graph with polar equation $=3-2\cos\theta$   md   
\end{example}
		\begin{center}
\resizebox{300pt}{!}{%
	\begin{tabular}{c|c|c|c|c|c|c|c|c|c}
